...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatIconModule, MatButtonModule, MatChipsModule, MatTooltipModule, MatProgressSpinnerModule],
  template: `
    <div class="flex flex-row gap-4 bg-surface-raised rounded-xl p-4 items-center flex-wrap">
      @if (forecastState.dataStatus(); as status) {
        ...
        <div class="flex items-center gap-3 flex-1 min-w-48">
          ...
          <button mat-icon-button matTooltip="Refresh Git data" class="text-content-muted flex-shrink-0"
            [disabled]="forecastState.isIngestionLoading()" (click)="refresh.emit('git')">
            @if (forecastState.isIngestionLoading()) {
        ...
        <div class="flex items-center gap-3 flex-1 min-w-48">
          ...
          <button mat-icon-button matTooltip="Refresh Jira data" class="text-content-muted flex-shrink-0"
            [disabled]="forecastState.isIngestionLoading()" (click)="refresh.emit('jira')">
            @if (forecastState.isIngestionLoading()) {
              <mat-spinner diameter="18"></mat-spinner>
            } @else {
              <mat-icon class="text-base">refresh</mat-icon>
        ...
        <div class="flex items-center gap-3 flex-1 min-w-48">
          ...
          <button mat-icon-button matTooltip="Refresh HRMS data" class="text-content-muted flex-shrink-0"
            [disabled]="forecastState.isIngestionLoading()" (click)="refresh.emit('hrms')">
            @if (forecastState.isIngestionLoading()) {
              <mat-spinner diameter="18"></mat-spinner>
            } @else {
              <mat-icon class="text-base">refresh</mat-icon>
        ...
        <!-- Refresh All -->
        <button mat-stroked-button class="flex-shrink-0"
          [disabled]="forecastState.isIngestionLoading()" (click)="refreshAll.emit()">
          @if (forecastState.isIngestionLoading()) {
            <mat-spinner diameter="18"></mat-spinner>
          } @else {
